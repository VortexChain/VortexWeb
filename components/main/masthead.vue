<template>
    <div
        class="ui inverted center aligned segment m-0 rounded-none bg-gray-900"
    >
        <sui-menu secondary inverted class="mb-0">
            ...
            <sui-menu-item
                ...
                ><n-link :to="localePath(item.route)">{{
                    item.title
                }}</n-link></sui-menu-item
            >
    ...
    methods: {
        arrowLeftOffsetCalc() {
            if (this.$refs['link' + this.acticeItemIndex]) {
                let activeLink = this.$refs['link' + this.acticeItemIndex][0]
                    .$el
    ...
    computed: {
        ...
        acticeItemIndex() {
            return this.links.findIndex(
                item =>
                    this.localePath(item.route) == this.$nuxt.$route.fullPath
            )
        }
    }
}
