<template>
    <div class="h-full flex items-stretch">
        <div class="flex-1 h-full flex flex-col items-stretch pr-3">
            <div
                ref="messagesArea"
                class="flex-1 overflow-auto flex flex-col pr-2 mb-3"
            >
                <div
                    v-for="(message, index) in messages"
                    :key="index"
                    v-ripple
                    :class="message.userId == currentUser.uid ? 'mine' : ''"
                ...
                >
                    ...
                    >
                        <sui-image
                            v-if="getMessageUser(message).photoUrl"
                            :src="getMessageUser(message).photoUrl"
                            shape="circular"
                            size="mini"
                        ...
                        <sui-icon
                            ...
                            style="height: 35px; width: 35px; font-size: 35px; line-height: 35px;"
                        />
                    </div>
                    <div>
                        <p class="font-bold">
                            {{
                                getMessageUser(message).name
                                    ? getMessageUser(message).name.split(' ')[0]
                                    : getMessageUser(message).email
                            }}
                        </p>
                        <p
        ...
        <div class="bg-gray-300 p-3 chat-clients">
            <div
                ...
                :key="'online' + index"
                v-ripple
                class="bg-gray-400 chat-user px-3"
            >
                <sui-image
            ...
            <div
                v-for="(client, index) in offlineUsers"
                :key="'offline' + index"
                v-ripple
                class="bg-gray-300 chat-user px-3"
            >
                <sui-image
                    v-if="client.photoUrl"
                    :src="client.photoUrl"
    ...
    computed: {
        ...
        offlineUsers() {
            return this.users.filter(user => {
                if (this.clients.find(client => client.uid == user.uid))
                    return false
                return true
            })
        }
    ...
    methods: {
        initChatConnection() {
            ...
            this.connection.on('ChatReady', data => {
                ...
                ;(this.clients = data.clients), (this.users = data.users)
            })

            this.connection.on('UserJoin', user => {
                this.clients.push(user)
                if (!this.users.find(client => client.uid == user.uid))
